...
Tous les objets sont chargés en RAM (mémoire vive).
Les tables sont traités comme des data.frame ou dérivés : tibble, data.table, sparseMatrix.
Les autres objets courants sont les vecteurs et les listes. Un data.frame est une liste de vecteurs de tailles identiques.

## Types de données
Les characters/chaînes de caractères.
Les facteurs ou variables catégorielle avec format (levels).
Les variables numériques.

## Les fonctions
Dans R, il n'y a que des fonctions (macro ?) à appliquer à des objets pour les transformer, les manipuler, calculer les stats...
Paramètres implicites vs explicites.
Différence entre <- et =
...
## Les packages
Si vous voulez faire quelque chose, quelqu'un l'a sans doute déjà codé dans une fonction.
Une fonction vient rarement seule, et pour assurer la bonne documentation et le maintien à jour des fonctions, on les encapsule dans des packages ou librairies (libraries).
Il ne vous reste qu'à trouver le package.
Pour bien comprendre, il faut savoir que n'importe qui peut développer une(des) fonction(s) et proposer un package R.
...
Dans R il y a des vecteurs partout, alors plutôt que d'écrire des boucles sur ces vecteurs, les fonctions sont naturellement généralisées pour effectuer des calculs sur l'ensemble des éléments du vecteur.
